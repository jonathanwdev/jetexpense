    ...
    LazyColumn(
        ...
        item {
            Box(
                ...
                val transProportions = items.extractProportion {
                    amounts(it)
                }
        ...
        items(items, key = key) { item ->
            ...
            val isItemDismissed = dismissState.isDismissed(DismissDirection.StartToEnd)
            val isDismissedToEnd = dismissState.targetValue == DismissValue.DismissedToEnd
            SwipeToDismiss(
                state = dismissState,
                background = {
                    AnimatedVisibility(visible = isDismissedToEnd) {
                        Surface(
                            modifier = Modifier
                                .fillMaxWidth()
...
fun AnimatedCircle(
    ...
    Canvas(modifier = modifier) {
        ...
        proportions.forEachIndexed { index, proportion ->
            ...
            drawArc(
                color = colors[index],
                startAngle = startAngle + DIVIDER_LENGTH_DEGREES / 2,
                sweepAngle = sweep - DIVIDER_LENGTH_DEGREES,
                topLeft = topLeft,
                size = size,
                useCenter = false,
                style = stroke
...
private const val DIVIDER_LENGTH_DEGREES = 1.8f
